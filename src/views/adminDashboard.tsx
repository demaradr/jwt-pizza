...
export default function AdminDashboard(props: Props) {
  ...
  async function filterUsers() {
    setUserList(await pizzaService.getUsers(userPage, 5, `*${filterUserRef.current?.value ?? ''}*`));
  }
                  ...
                  <div className="overflow-hidden">
                    <table className="min-w-full divide-y divide-gray-200">
                      ...
                      <tbody className="divide-y divide-gray-200">
                        {userList.users.map((u, idx) => (
                          <tr key={u.id ?? idx} className="border-neutral-500 border-t-2">
                            ...
                            <td className="text-start px-2 whitespace-nowrap text-sm text-gray-800">
                              {u.roles?.map((r) => formatRole(r)).join(', ') ?? ''}
                            </td>
                            <td className="px-6 py-1 whitespace-nowrap text-end text-sm font-medium">
                              <button
                                type="button"
